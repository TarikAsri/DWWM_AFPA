...
------------------------------ T R A V A I L - A - R E A L I S E R ------------------------------
USE hotel; 
-- 1.
...
INNER JOIN chambre
ON res_cli_id = cha_id
INNER JOIN hotel
ON cha_hot_id = hot_id;

-- 11.
SELECT hot_nom, cha_numero, cha_capacite,  sta_nom
FROM hotel
INNER JOIN chambre
ON cha_hot_id = hot_id
INNER JOIN station
...
-- 12.
SELECT  res_date_debut,DATEDIFF( res_date_fin,  res_date_debut), hot_nom, cli_nom 
FROM client 
JOIN reservation 
...
--16.
SELECT cli_id, cli_nom, res_cli_id, res_cha_id, cha_hot_id, hot_nom, hot_id 
FROM  client
JOIN reservation 
ON res_cli_id = cli_id 
JOIN chambre 
...
--17.
SELECT DATEDIFF( res_date_fin, res_date_debut) 
FROM client
JOIN reservation 
ON res_cli_id = cli_id  
JOIN chambre 
